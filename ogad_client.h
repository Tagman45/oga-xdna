/**
 * ogad_client.h — Client C++ pour OGA Daemon (ogad_v3.py).
 *
 * Communication JSON ligne par ligne via stdin/stdout du sous-processus.
 *
 * Architecture reelle (verifiee ProcMon):
 *   llama.cpp → ogad_client.h → ogad_v3.py → ONNX Runtime GenAI
 *       → DirectML.dll → D3D12.dll → dxgkrnl.sys → NPU XDNA2
 *
 * La chaine RadeonML (RadeonML_ipu.dll, maize.dll, tvm_runtime.dll)
 * N'EST PAS utilisee. Le NPU est compatible DirectML nativement.
 *
 * Usage:
 *   OGADClient client;
 *   client.start("python", {"ogad_v3.py"});  // lance le daemon
...
public:
    ...
    bool start(const std::string& python_exe = "python",
               const std::string& script = "C:\\trixdna_test\\ogad_v3.py") {
    ...
    OGACommandResult reset(const std::string& model_path = "") {
        std::string cmd;
        if (model_path.empty())
            cmd = "{\"cmd\":\"reset\"}\n";
        else
            cmd = "{\"cmd\":\"reset\",\"model\":\"" + model_path + "\"}\n";
        return send_and_recv(cmd);
    }
